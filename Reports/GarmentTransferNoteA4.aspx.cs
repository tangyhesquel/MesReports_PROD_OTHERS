...
public partial class Reports_GarmentTransferNoteA4 : pPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (this.Page.Request.QueryString.Get("detail") == "Y")
            {
    ...
    private void ShowAllButYMG()
    {
        ...
        if (Process_CD == "CUT" || Process_CD == "PRT" || Process_CD == "CUT2")
        {
    ...
    protected void summaryOnly_checkChanged(object sender, EventArgs e)
    {
        if (Request.QueryString.Get("site") == null)
            return;
    ...
    protected void RepeaterCut_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        {
            tb2.Rows.Add(row.JOB_ORDER_NO, row.TOTAL_CUT_QTY, 0, row.TOTAL_OUTPUT_QTY, (Convert.ToInt32(row.TOTAL_CUT_QTY) - Convert.ToInt32(row.TOTAL_OUTPUT_QTY)).ToString());
        }
        ...
        detailHeader["COLOR_CD"] = tb2.Rows[count]["JOB_ORDER_NO"].ToString();
        ...
        for (int i = 0; i < tb3.Rows.Count; i++)
        {
            ...
            if (tb3.Rows[i]["JOB_ORDER_NO"].ToString() == tb2.Rows[count]["JOB_ORDER_NO"].ToString())
            {
                ...
                if (i == tb3.Rows.Count - 1)
                {
                    ...
                    detailData["COLOR_CD"] = "";
                    detailData["BUNDLE_NO"] = bundle_qty;
                    detailData["SIZE_CD"] = "";
                    detailData["QTY"] = tb2.Rows[count]["CURRENT_OUTPUT"].ToString();
        ...
        for (int i = 0; i < gv1.Rows.Count; i++)
        {
            if (gv1.Rows[i].Cells[0].Text == "HEADER")
            {
                for (int j = 0; j < gv1.Rows[i].Cells.Count; j++)
                {
                    ...
                    gv1.Rows[i].Cells[j].BackColor = System.Drawing.Color.Azure;
                }
            }
        ...
        for (int i = 0; i < gv2.Rows.Count; i++)
        {
            if (gv2.Rows[i].Cells[5].Text == "HEADER")
            {
                for (int j = 0; j < gv2.Rows[i].Cells.Count; j++)
                {
                    ...
                    gv2.Rows[i].Cells[j].BackColor = System.Drawing.Color.Azure;
                }
            }
            ...
            for (int i = 0; i < gv3.Rows.Count; i++)
            {
                if (gv3.Rows[i].Cells[5].Text == "HEADER")
                {
                    for (int j = 0; j < gv3.Rows[i].Cells.Count; j++)
                    {
                        ...
                        gv3.Rows[i].Cells[j].BackColor = System.Drawing.Color.Azure;
                    }
                }
    ...
    private DataTable FormatDataTable_cut(DataTable summaryTable, int SizeCountPerRow)
    {
        ...
        //if ((summaryTable.Columns.Count - 4) % SizeCountPerRow == (SizeCountPerRow - 2))
        //{
        //    rowCount = rowCount + 1;
        //}
        if ((summaryTable.Columns.Count - 4) % SizeCountPerRow == (SizeCountPerRow - 1) && (sizeCols != (SizeCountPerRow - 1))) //Modified by MF on 20140903, Display error when SizeCol = 9
        {
            rowCount = rowCount + 1;
            nextLine = true;
        }
        ...
        if (sizeCols == SizeCountPerRow && nextLine == false && rowCount == 0)
        {
            table.Columns.Add("TOTAL", typeof(string));
            table.Columns.Add("TTL_BY_GO_COLOR", typeof(string));
        }
        ...
        for (int i = 0; i <= rowCount; i++)
        {
            for (int j = 3; j < SizeCountPerRow + 3; j++)
            {
                if (i == rowCount && nextLine == true)
                {
                    ...
                    rowHeader["SIZE0"] = Resources.GlobalResources.STRING_TTL_GO_COLOR;
                    break;
                }
                else
                {
            ...
            for (int k = 0; k < summaryTable.Rows.Count; k++)
            {
                ...
                for (int l = 3; l < SizeCountPerRow + 3; l++)
                {
                    ...
                    else if ((l + a) == summaryTable.Columns.Count)
                    {
                        if (nextLine == true && i == rowCount)
                        {
                            rowData["SIZE" + (l - 3)] = byColor.ToString();
                            break;
                        }
                    }
